...
class Mesh3D:
    ...
    def load_textured_obj(obj_file, texture) -> "Mesh3D":
        ...
        vertex_buffer = []
        # vertex_buffer will contain 5 values per vertex: x, y, z, u, v
        ...
        vertex_textures : dict[int, dict[int, int]] = {}
        ...
        for i in range(0, len(faces), 9):
            ...
            v1_texindexes = vertex_textures.setdefault(v1_index, {texindex_v1 : v1_index})
            v2_texindexes = vertex_textures.setdefault(v2_index, {texindex_v2 : v2_index})
            v3_texindexes = vertex_textures.setdefault(v3_index, {texindex_v3 : v3_index})
            ...
            # for vertex 1.
            if texindex_v1 not in v1_texindexes:
                vertex_buffer.append(vertex_buffer[v1_index * 5])
                vertex_buffer.append(vertex_buffer[v1_index * 5+1])
                vertex_buffer.append(vertex_buffer[v1_index * 5+2])
                vertex_buffer.append(texcoords[2 * texindex_v1])
                vertex_buffer.append(texcoords[2 * texindex_v1+1])
                v1_index = len(vertex_buffer)//5-1
                v1_texindexes[texindex_v1] = v1_index
            else:
                v1_index = v1_texindexes[texindex_v1]
                vertex_buffer[v1_index * 5 + 3] = texcoords[2 * texindex_v1]
                vertex_buffer[v1_index * 5 + 4] = texcoords[2 * texindex_v1 + 1]

            if texindex_v2 not in v2_texindexes:
                vertex_buffer.append(vertex_buffer[v2_index * 5])
                vertex_buffer.append(vertex_buffer[v2_index * 5+1])
                vertex_buffer.append(vertex_buffer[v2_index * 5+2])
                vertex_buffer.append(texcoords[2 * texindex_v2])
                vertex_buffer.append(texcoords[2 * texindex_v2+1])
                v2_index = len(vertex_buffer)//5-1
                v2_texindexes[texindex_v2] = v2_index
            else:
                v2_index = v2_texindexes[texindex_v2]
                vertex_buffer[v2_index * 5 + 3] = texcoords[2 * texindex_v2]
                vertex_buffer[v2_index * 5 + 4] = texcoords[2 * texindex_v2 + 1]
            
            if texindex_v3 not in v3_texindexes:
                vertex_buffer.append(vertex_buffer[v3_index * 5])
                vertex_buffer.append(vertex_buffer[v3_index * 5+1])
                vertex_buffer.append(vertex_buffer[v3_index * 5+2])
                vertex_buffer.append(texcoords[2 * texindex_v3])
                vertex_buffer.append(texcoords[2 * texindex_v3+1])
                v3_index = len(vertex_buffer)//5-1
                v3_texindexes[texindex_v3] = v3_index
            else:
                v3_index = v3_texindexes[texindex_v3]
                vertex_buffer[v3_index * 5 + 3] = texcoords[2 * texindex_v3]
                vertex_buffer[v3_index * 5 + 4] = texcoords[2 * texindex_v3 + 1]
